  ...
  <el-row class="search-row">
    <div class="search-item">
      ...
      <div style="width: 150px">
        <el-input clearable placeholder="请输入课程名称" v-model="searchForm.name" maxlength="18"/>
      </div>
    </div>
    
    <button class="x-btn x-btn-m" type="primary" @click="loadThisPage">查询</button>
    <button class="x-btn x-btn-success x-btn-m" v-if="user && ( user.role.toLowerCase() == 'admin'   )" @click="onAdd">新增课程</button>
...
<script setup name="course">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/course/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/course/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    //新增页
    const onAdd = async () => {
        const op = Dialog.open(AddPage, `新增课程`).setConfirmText('保存')
        op.mounted(c => {
    ...
    const onDelete = async (id) => {
        ...
        op.then(async () => {
            let {success, message} = await Http.post(`/course/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
