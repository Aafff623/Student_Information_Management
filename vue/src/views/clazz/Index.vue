  ...
  <el-row class="search-row">
    <div class="search-item">
      ...
      <div style="width: 150px">
        <el-input clearable placeholder="请输入班级名" v-model="searchForm.name" maxlength="18"/>
      </div>
    </div>
...
<script setup name="clazz">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/clazz/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/clazz/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    const onDelete = async (id) => {
        ...
        op.then(async () => {
            let {success, message} = await Http.post(`/clazz/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
