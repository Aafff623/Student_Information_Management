<template>
  ...
  <el-row class="search-row">
    <div v-if="user.role!=='student'" class="search-item">
      <span>学生</span>
      <div style="width: 150px">
        <x-select-table :header="[{label:'学生',field:'name',width:'100%'}]"
    ...
    <div v-if="user.role!=='course'" class="search-item">
      <span>课程</span>
      <div style="width: 150px">
        <x-select-table :header="[{label:'课程',field:'name',width:'100%'}]"
    ...
    <button class="x-btn x-btn-success x-btn-m" v-if="user && ( user.role.toLowerCase() == 'admin'   )" @click="onAdd">新增成绩记录</button>
  ...
  <el-table :data="pager.records" style="width: 100%">
    <el-table-column label="学生">
      <template #default="scope">
        <el-button link v-if="scope.row.sidFrn" type="primary" text bg @click="sidDetail(scope.row.sid)">{{ scope.row.sidFrn.name }}</el-button>
      </template>
    </el-table-column>
    <el-table-column label="课程">
      <template #default="scope">
        <el-button link v-if="scope.row.cidFrn" type="primary" text bg @click="cidDetail(scope.row.cid)">{{ scope.row.cidFrn.name }}</el-button>
      </template>
    </el-table-column>
...
<script setup name="score">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/score/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/score/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    //新增页
    const onAdd = async () => {
        const op = Dialog.open(AddPage, `新增成绩记录`).setConfirmText('保存')
        op.mounted(c => {
    ...
    const onDelete = async (id) => {
        ...
        op.then(async () => {
            let {success, message} = await Http.post(`/score/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
